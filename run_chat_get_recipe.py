...
def load_console():
    from services.hci.console import ConsoleInput, ConsoleOutput # copied from adviser.services.hci.speech
...
def load_asr(use_cuda: bool, conversation_log_dir: str = None, voice_privacy: bool = False):
    from services.hci.speech.SpeechInputDecoder import SpeechInputDecoder # copied from adviser.services.hci.speech
    from services.hci.speech.SpeechInputFeatureExtractor import SpeechInputFeatureExtractor # copied from adviser.services.hci.speech
    from services.hci.speech.SpeechRecorder import SpeechRecorder # copied from adviser.services.hci.speech
...
def load_tts(use_cuda: bool, conversation_log_dir: str = None): # copied from adviser.services.hci.speech (from a path of a folder)
    from services.hci.speech.SpeechOutputGenerator import SpeechOutputGenerator
    from services.hci.speech.SpeechOutputPlayer import SpeechOutputPlayer
...
def load_bachchannel():
    from services.backchannel.acoustic_backchannel import AcousticBackchanneller # copied from adviser.services.backchannel (from a path of a folder)
...
def load_gui(): # not sure if we need this part
    from services.hci.gui import GUIServer # copied from adviser.services.hci.gui
    return GUIServer()
...
def load_soup_domain(backchannel: bool = False):
    from utils.domain.jsonlookupdomain import JSONLookupDomain
    from services.nlu.nlu import HandcraftedNLU
    from services.nlg.nlg import HandcraftedNLG
    from services.policy.policy import HandcraftedPolicy
    domain = JSONLookupDomain('Soup', display_name="soup")
    soup_nlu = HandcraftedNLU(domain=domain)
    soup_bst = HandcraftedBST(domain=domain)
    soup_policy = HandcraftedPolicy(domain=domain)
    soup_nlg = load_nlg(backchannel=backchannel, domain=domain)
    return domain, [soup_nlu, soup_bst, soup_policy, soup_nlg]
...
if __name__ == "__main__":
    ...
    ## modified this part
    if 'soup' in args.domains:
        l_domain, l_services = load_soup_domain(backchannel=args.bc)
