...
class HandcraftedNLU(Service):
    ...
    def _match_domain_specific_act(self, user_utterance: str):
        ...
        # Find Informs
        ## Recognize the number and trun it into integer
        if user_utterance.isdigit():
            self._match_inform(int(user_utterance))
        else:
    ...
    def _add_request(self, user_utterance: str, slot):
        """
    ...
    def _add_inform(self, user_utterance, slot, value):
        """
        Creates the user request act and adds it to the user act list
